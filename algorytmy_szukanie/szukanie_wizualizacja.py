...
file.sort()
lista_losowa = [random.choice(file) for _ in range(RAN_LIS_LEN)]  
...
binarne = [reduce(lambda x,y:x+y,x,0)/RAN_LIS_LEN for x in total_binarne]
liniowe = [reduce(lambda x,y:x+y,x,0)/RAN_LIS_LEN for x in total_liniowe]
binarne_czas = [reduce(lambda x,y:x+y,x,0)/RAN_LIS_LEN for x in total_binarne_czas]
liniowe_czas = [reduce(lambda x,y:x+y,x,0)/RAN_LIS_LEN for x in total_liniowe_czas]
...
axs[2,0].plot([sum(total_binarne[0])/RAN_LIS_LEN for _ in range(0,RAN_LIS_LEN+1)],color="red")
...
axs[2,1].plot([sum(total_liniowe[0])/RAN_LIS_LEN for _ in range(0,RAN_LIS_LEN+1)],color="red")
